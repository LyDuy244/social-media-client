...
const ImageEditor = ({onClose, settings, setSettings, previewUrl}: {
  onClose: () => void;
  settings: {
  ...
  const handleChangeType = (type: "original" | "square" | "wide") => {
    setSettings(prev => ({
      ...prev,
      type,
    }))
  ...
  return (
    <div
      className={"fixed w-screen h-screen left-0 top-0 bg-black bg-opacity-75 z-10 flex items-center justify-center"}>
      <div className={"bg-black rounded-xl p-8 flex flex-col gap-4"}>
        {/*TOP*/}
        <div className={"flex items-center justify-between"}>
          ...
          <button className={"px-4 py-2 rounded-full bg-white text-black font-bold"} onClick={onClose}>Save</button>
        </div>
        {/*  IMAGE CONTAINER*/}
